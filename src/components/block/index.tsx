...
const Block: React.FC<{ text: string, id: string, clickable?: Boolean, href?: string }> = ({ text, id, clickable, href }) => {
      ...
      if (clickable) {
        ...
        if (outline) {
          ...
          block.addEventListener('mouseup', () => {
            ...
            root.style.setProperty("--outline-to-width", blockPos.width + blockClickOffset + "px")
            root.style.setProperty("--outline-to-height", blockPos.height + blockClickOffset + "px")
            root.style.setProperty("--outline-to-top", blockPos.top - (blockClickOffset / 2) + "px")
            root.style.setProperty("--outline-to-left", blockPos.left - (blockClickOffset / 2) + "px")
          })
        }
      }
    }
  ...
  return (
    <>
      {/* <ViewTransition update="none" name={"outline-" + id}> */}
      {/*   <span style={{ viewTransitionName: 'outline-' + id }} className={styles.outline} ref={outlineRef} /> */}
      {/* </ViewTransition> */}
      <ViewTransition name={id}>
        <span style={{ viewTransitionName: id }} id={id} className={styles.block} onClick={onClick} ref={blockRef}>
          <h3>{text}</h3>
        </span>
      </ViewTransition>
    </>
